...
namespace aspnetcore_template.MVC.Mappings
{
    public class RestaurantTransforms
    {
        public static string ProcessServerActionsAnchor(int id)
        {
            ...
            sb.AppendFormat(
                "<a id='{0}' href='#' onclick='event.preventDefault();UpdateConfigMaint(this); return false; '<i class='glyphicon-margin glyphicon glyphicon-floppy-disk glyphicon-margin' style='color: green' data-toggle='tooltip' title='Update Selected Rows'></i></a>&nbsp;", id);
            sb.AppendFormat(
